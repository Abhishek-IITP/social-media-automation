...
const getOrCreateZernioProfile =async(user:any): Promise<void>=>{
     try {
          ...
          console.error("Error from getOrCreateZernioProfile controller", error?.message)
     }
}
...
export const syncAccounts = async(req:AuthRequest ,res:Response): Promise<void> =>{
     try {
          ...
          for(const zAccount of zernioAccounts){
               ...
               const normalizaedPlatform = supportedPlatforms.find((p)=>rawPlatform.includes(p));

               if(!normalizaedPlatform){
