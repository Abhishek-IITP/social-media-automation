...
export function Sidebar({isOpen, setIsOpen}:{isOpen:boolean,setIsOpen: (val: boolean)=>void}){
     ...
     const {logout,user} ={
          ...
          user: {
               name :"John Doe", email :"abhi@gamil.com"
          }
     }
     ...
     return <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${isOpen? "translate-x-0": "-translate-x-full"}`}>
          ...
          <nav className=" flex-1 px-3 space-y-1 ">
               {navItems.map((item)=>{
                    ...
                    return(
                         ...
                         className={`flex gap-4 items-center px-3 py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? 'bg-red-50 text-red-600 border-red-100' : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"} `}
                         >
